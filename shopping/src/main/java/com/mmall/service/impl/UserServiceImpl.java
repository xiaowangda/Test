...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse selectQuestion(String username) {
        ...
        String question = userMapper.serlectQuestionByUsername(username);
    ...
    public ServerResponse<String> checkAnswer(String username, String question, String answer) {
        int resultCount = userMapper.checkAnswr(username, question, answer);
        if (resultCount > 0) {
            //说明问题及问题答案是这个用户的吗，并且是正确的
    ...
    public ServerResponse<String> forgetRestPassword(String username,String passwordNew,String forgetToken){
        ...
        if(org.apache.commons.lang3.StringUtils.isBlank(token)){
            return ServerResponse.createByErrorMessage("token无效或者到期");
        }
    ...
    public ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //emil也要进行一个校验，校验新的email是不是已经存在，并且存在的email如果相同的话，不能是我们当前的这个用户的
        ...
        if(updateCount>0){
            return ServerResponse.createByErrorMessage("更新个人信息成功"+updateUser);
        }
